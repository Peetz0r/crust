...
int
css_set_css_state(unused uint8_t state)
{
	/* Nothing to do. */
	return SCPI_OK;
}
...
css_set_cluster_state(uint8_t cluster, uint8_t state)
{
	...
	if (state == CSS_POWER_ON) {
		debugS("Turning CSS cluster on...");
		...
		/* FIXME: This is a temporary hack to hopefully crash less. */
		udelay(50);
		debugS("done!");
	} else if (state == CSS_POWER_OFF) {
		debugS("Turning CSS cluster off...");
