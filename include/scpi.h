...
/**
 * This SCPI implementation has two clients: Linux and ATF. CPU/cluster and
 ...
 */
enum {
	SCPI_CLIENT_SECURE = 0, /**< Client 0: Trusted Firmware (secure). */
	SCPI_CLIENT_NS     = 1, /**< Client 1: Linux (non-secure). */
	SCPI_CLIENTS       = 2, /**< The number of supported SCPI clients. */
};
...
/**
 * The set of possible system power states, defined by the SCPI specification.
 */
enum {
	SCPI_SYSTEM_POWER_SHUTDOWN = 0,
	SCPI_SYSTEM_POWER_REBOOT   = 1,
	SCPI_SYSTEM_POWER_RESET    = 2,
};
...
/**
 * The structure representing a SCPI shared memory area, defined by the SCPI
 * specification.
 */
struct scpi_mem {
...
/**
 * Handle a received SCPI message. This function parses the message, performs
 * any requested actions, and possibly generates a reply message.
 *
 * @param  chan  The mailbox channel where the message was received.
 * @param  msg   The message received in the mailbox.
 */
void scpi_receive_message(uint8_t chan, uint32_t msg);
